---
title: 'Script 1: Intro to R and exploring data with graphs: Solutio'
...
- **Exercise:** Set up your own working directory. Uncomment the code below to complete the task
```{r}
#setwd()
```
...
```{r metallica_vector4}
metallica[1] <- "lars - drummer"
metallica[2] <- "james - lead vocals"
metallica[3] <- "Kirk - lead guitar"
metallica[4] <- "Rob - bass"

# Check out the contents of the vector again
metallica
```
...
metallica_list <- list(metallicaNames, metallicaAges)
...
metallica_list[[1]][1] # you would get the output : "Lars" - the first element of the first level of the list, which is the vector including the band members' names
metallica_list[[2]][3] # you would get the output : 60 - the third element of the second level of the list, which is the vector including the band members' ages
```
...
metallica_list[[1]][1] <- "Lars Ulrich"
metallica_list[[1]][2] <- "James Hetfield"
metallica_list[[1]][3] <- "Kirk Hammett"
metallica_list[[1]][4] <- "Rob Trujillo"
...
```{r metallica_matrix1}
metallica_matrix <- cbind(metallicaNames, metallicaAges)
```
...
```{r metallica_matrix2}
class(metallica_matrix) # the class is matrix, array

typeof(metallica_matrix) # the type of the content within the array is character
```
...
metallica_matrix[1,1] #Lars
metallica_matrix[1,2] #59

# notice the similarities and differences in indexing across different data structures
```
...
```{r data_frames4}
metallica_df$newChildAge <- metallica_df$childAge * 2
```
...
```{r hist1}
ggplot(data = chickflick_data, aes(x = arousal, y = film))
```
...
```{r hist4}
ggplot(data = chickflick_data, aes(x = arousal))+
  geom_histogram(color = "lightpink4", fill = "lightpink2", bins = 10) +
  facet_grid(~gender)+
  labs(x = "Average physiological arousal", y = "Frequency")+
  labs(title = "Average physiological arousal as a function of gender")
```
...
Example: Overall the arousal scores for "Memento" were much higher for both female and male participants than for "Bridget Jones' diary". In both cases the highest average arousal score was about 40. The most frequent arousal scores registered observed for the Memento movie for female participants were registered around values of approx. 15, 20 and 30. The most frequent arousal scores registered observed for the Memento movie for female participants were registered around values of approx. 17, 21, 25. The average male arousal scores for Bridget Jones' diary were higher than for female participants. The most frequent arousal scores for Bridget Jones' diary as observed for male participanrs were approx. 10 and 21, while for female participants approx 15.
...
geom_bar(position = "dodge",
           ...
           color="black")+    
  labs(x = "Film", y = "Average physiological arousal", fill = "Film",
       title = "Average physhiological arousal as a function of gender")+ # renaming the x and y axis, the colour legend, creating a title
...
The average arousal was overall higher for male vs. female participants for both films, yet the difference between male and female participants was larger for Bridget Jones' Diary rather than for Memento. As well as that, the movie Memento triggered on average more physiological arousal compared to Bridget Jones' diary. The error bars inform that the means associated with the average arousal for Memento overlap almost completely. This suggests that the male and female participants were similar in their physiological reactions to the Memento movie. By contrast, there is a visibly larger difference between the average physiological arousal scores between male and female participants who watched Bridget Jones' Diary.  
...
**Plotting the distribution of arousal scores as a function of gender**

-**Excercise**: It's your turn to build a boxplot. Plot the the average physiological arousal as a function of gender.

```{r boxplot2}
ggplot(data = chickflick_data, aes(x = gender, y = arousal, fill = gender))+
  geom_boxplot() +
  labs(x = "Film", y = "Average physiological arousal", 
       title = "Average physhiological arousal as a function of gender")+ 
...
```{r boxplot3}
ggplot(data = chickflick_data, aes(x = gender, y = arousal, fill = film))+
  geom_boxplot() +
  labs(x = "Film", y = "Average physiological arousal", 
       title = "Average physhiological arousal as a function of film and gender")+ 
  scale_fill_brewer(palette = "Pastel2")
```
...
Since boxplots help us learn about the distribution of the data, as well as the variability of it, we learn right off the bat that there is visible more variability within the Memento gender subgroups compared to the Bridget Jones' Diary - this is suggested by the size of the boxes. The medians associated with the male gender subgroups suggest that the median physiological arousal was higher for male vs. female participants, which is something we have seen in previous plots as well. Notice the large and overlapping error bars associated with the Memento male and female groups - this is an indicator that the difference in arousal between male and female participants could have been rather small when having watched Memento.
