...
knitr::opts_chunk$set(
  fig.width  = 8, 
  fig.height = 5, 
  fig.path   = '/Users/anamariaplesca/Desktop/Stats planning/inspiration courses/docs 2/01_data_wrangling/morning_intro_to_R',
  echo       = TRUE, 
  warning    = FALSE, 
  message    = FALSE,
  cache      = TRUE
...
- **Exercise:** Let's add the band members' surnames. Use indexing and the assignment operator `<-` to replace the existing values. (If you feel stuck, scroll back up to the section about variables.)
...
- **Exercise:** Use the function `cbind()` to combine the `metallicaNames` and `metallicaAges`. Store the result in a variable named `metallica` The end result should look like this:
...
- **Exercise:** Try indexing your `metallica` array. What will show up if you index metallica at [1,1], what about [1,2]? Uncomment the line codes and then add comments of the results.
