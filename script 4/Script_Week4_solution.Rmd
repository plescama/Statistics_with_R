...
hunger_data <- data <- data.frame(hungerScore = c(1, 2, 8, 3, 7, 9, 9, 7, 8, 9),
...
- 1 The values along the diagonals represent the variances of each variable:
...
- A coefficient of +1 indicates that the two variables are perfectly positively correlated, and as one variable increases, the other one increases as well.
...
<div class="alert alert-warning" role="alert">
If you are not sure what these data types mean (ratio, interval, etc.), here's a chance to refresh these notions and see some other examples.
</div>
...
```{r histogram_ads}
hist(commercial_sweets_dat$ads_watched)
```

- **Exercise**: Plot a histogram to show the distribution of the packets of sweets that were bought

```{r histogram_sweets}
hist(commercial_sweets_dat$packets_bought)
```

- **Exercise**: Plot a QQ-plot for the watched commercials

```{r qqplot_ads}
# Run both lines of code together. Select them and them run
qqnorm(commercial_sweets_dat$ads_watched)
qqline(commercial_sweets_dat$ads_watched)
```
...
```{r qqplot_sweets}
# Run both lines of code together. Select them and them run
qqnorm(commercial_sweets_dat$packets_bought)
qqline(commercial_sweets_dat$packets_bought)
```
...
Based on the QQ-plots it becomes quite clear that both variables seem to be close to the normal distribution. However, this pattern is not as easily noticeable based on the histograms only. This is why it's important to have more methods of visualisation.
...
```{r shapiro_wilk_ads}
shapiro.test(commercial_sweets_dat$ads_watched)
```
...
shapiro.test(commercial_sweets_dat$packets_bought)
```
...
Depending on which data you wish to analyse, the correlation coefficient which you would might want to put up for a statistical test will be different. Here are some very helpful excerpts from Chapter 6 from Andy Field's "Discovering statistics with R":
...
>Imagme I wanted to test a theory that more creative people will be able to create taller
...
We can use the correlation coefficient to gain even more information. If we square it, we get the **coefficient of determination, R^2**. This is a measure of the amount of variance in a variable that is shared by another variable.
...
```
Both variables that we wish to test for correlation yielded non-significant, which indicates that they are normally distributed. Therefore, we can use the Pearson's correlation coefficient test.

Before we run it, let's say we decide for a one-tailed alternative hypothesis stat states: There will be a positive relationship between the two variables. The more hours invested in writing the essay, the better the grade will be.
...
The test yielded a significant result, whereby Pearson's correlation coefficient of r = .26, p < .05 suggests there is a positive relationship between the two variables. The magnitude of the correlation coefficient informs us that there is a relationship of medium strength between the hours invested in writing an essay and the obtained grade, as informed by scores in percentages.

## Exploring the relationship between and hours and essay grade degree classes

Were you tempted to conduct a normality test? Well, considering that the values in the `grade` column represent ordinal data, there is no need to do this. Because we are dealing with ordinal (ranked, ordered) data we can use either Spearman's rho or Kendall's tau. Since our sample is of 45 participants, we can use Spearman's rho, but we will run both so we can compare results.


```{r correlation_analysis2}
cor.test(essayData$essay, essayData$hours, alternative = "greater", method = "spearman")
```

```{r}
cor.test(essayData$essay, essayData$hours, alternative = "greater", method = "kendall")

```

Both analyses provide us with essentially the same result, though the estimates (that is the estimated correlation coefficients) are different. This makes sense, considering that Kendall's tau is meant for smaller sample sizes (under 30) and Spearman for larger sample sizes.
