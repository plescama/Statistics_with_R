---
title: 'Script 2: Learning about measures of central tendency, statistical models and assumptions'
...
2. Discovering [statisical models](#stat_models) and [assessing model fit based on plots](#model_fit)
3. Identifying and dealing with [outliers](#outliers) using visualisation and [z-scores](#zscores)
4. Learning about the [assumptions underlying parametric tests](#assumptions) and how to test for these assumptions using R. 
R-related skills: 
1. Introducing [relational operators](#relational_operators).
2. Filtering and summarizing data.
...
This week, you will learn to look at your data it via measures of central tendency. We will introduce the notion of outliers and we'll learn how to identify them. In addition to that, we will briefly discuss the notion of a statistical model and we will be showing what a very simple model looks like. In order to put your data to statistical tests, you will also learn how to assess whether it meets certain assumptions that need to be fulfilled in oder to perform such tests. 
...
# Rememeber that if you are using a laptop/PC running on Windows, you need to change the direction of the slashes in the path!
```
...
Measures of central tendecy such as **the mean**, **the median**, **the mode** describe our data and inform about the centre of its distribution.
...
The mean is the average value of the dependent, numerical variable we've observed. Remember that it's a single value representing an entire dataset, and it is  very sensitive to extreme values (or outliers). Computing the mean in R is straightforward:
...
- **Exercise** Find out the average number of friends and the average neuroticism score. Save the results in separate variables with names of your chosing.

```{r mean2}
mean_no_friends <- mean(data$Friends)
mean_neuroticism <- mean(data$Neurotic)
```

## The median

Much like the mean, the median represents the data with a single value. By contrast, it is less sensitive to atypical, extreme values. Here's how to compute it:
...
Let's compare the mean income and the median income. We can do that with the help of relational operators. These will prove to be an useful tool for data exploration.
...
Is the mean income smaller than the median income?
...
**Step 2: Check whether the column contains values of numerical type.**

This is necessary because we want to compare numerical values and not characters. Use the `typeof()` function or take a look at the values stored in your R environment.
...
Note: We will introduce the pipe operator `%>%` (`Ctrl`+`Shift`+`M` - Windows, `Cmd` + `Shift` + `M` - Mac). This operator is used with the `tidyverse` package and allows us to pre-process data easily. 
...
Let's say we would like to find a way to summarize the observations in the `Chickflick` data, in order to be able to speak about a general tendency of the physiological arousal scores. To do this, we could build a statistical model, based on which we could also make predictions about the population. For instance, given the model, and all we know about the `Chickflick` data based on the sample we're working with, we could predict that a male participant that watches Memento would experience more arousal than a female watcher. 

For the purpose of the following example, we will be using the mean as a very basic model for the arousal scores, averaged over gender and film. In what follows you will see a visualization the mean as a model by way of a scatterplot. The mean as a model will be represented by a horizontal red line.
...
**Exercise**: Add a column z_scores to the chickflick_outliers dataset. Use the variables we have created above and the z-score formula.
...
**Exercise**: Filter the data to find values that have z-scores higher than 2 and save the result in a separate variable.
...
If you have a good reason to remove outliers, here is how you could do it. 

**Exercise:** Use the pipe operator `%>%` and the `filter()` function we've encountered before. To exclude the extreme scores all you have to do is use an additional `!` before the argument of the filter function. You are instructing the function to filter out all data points that fulfill the condition you typed as an argument to the `filter` function
...
*For dependent samples*:
  ...
  - **Sphericity** (more about this later in the course. If you are really curious, take a look here https://methods.sagepub.com/reference/encyc-of-research-design/n430.xml - it is an assumption similar to that of homogeneity of variance for independent samples.)

------------------------

## Checking the assumptions

We will be working with the `Chickflick` data, more exactly with its simplified version. In what follows we will test whether the data meets the assumptions for an **independent parametric test**, since we want to compare the female and male gender groups. Later on, we will also learn to compare the assumptions for dependent parametric tests
...
In the case of our dataset we can be fairly sure that the errors around the means of interest (for each gender) are independent within a group and between groups. Each group saw the movies separately and within each group the participants saw the movies separately. 
...
Please see the second class handout for an explanation of the Lilliefors-corrected Kolmogorov–Smirnov test. Keep in mind that this test is suitable for larger sample sizes (above 30). To implement it, we will use the function `lillie.test()` from the `nortest` package and we'll call the function as such: `nortest::lillie.test()`
...
Congratulations on completing your second R script! 
